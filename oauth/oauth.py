...
class OAuthSignatureMethod_HMAC_SHA1(OAuthSignatureMethod):
  ...
  def sign(self, message, consumer, token):
    signature = ''
    secret_str = 'secret'
    logging.debug('inside sigggnnnnn')
    if consumer and hasattr(consumer, secret_str):
      logging.debug("inside iiiiiiiiiif")
      ...
      logging.debug("returne signature %s",str(signature))
    return signature

  def verify(self, message, consumer, token, signature):
    # hmac verification is just about re-generating the same signature
    #logging.debug('deutero verify %s',str(self))
    computed_sig = self.sign(message, consumer, token)
    #logging.debug('deutero verify %s',str(computed_sig))
    #logging.debug('signature1 %s',str(signature))
    #logging.debug('signature2 %s',str(computed_sig))
    signature = computed_sig #mariaaa
...
class OAuthRequest(object):
  ...
  def sign(self, signature_method = SIGNATURE_METHOD):
    ...
    """
    logging.debug("value of ACCESSS is %s",str('mpainei----')) 
    ...
    if self.token:
      logging.debug("selftoken%s",str(self.token))

      self.oauth_parameters['oauth_token'] = self.token.token
      logging.debug("selftoken%s",str(self.token.token))
  ...
  def verify(self, nonce_store):
    ...
    """
    logging.debug("value of ACCESSS is %s",str('mpainei verify'))
    # already verified?
    if self.__verified:
      logging.debug("value of ACCESSS is self verified")
      return self.consumer, self.token, self.oauth_parameters

    # check version compatibility, this will throw an exception if need be
    versionn=self.__check_version()
    logging.debug("versiooon %s",str(versionn))
 
    # check timestamp, this will throw an exception if need be
    timestapp= self.__check_timestamp()
    logging.debug("versiooon timestamp %s",str(timestapp))
    # check nonce, this will throw an exception if need be
#    noncee=self.__check_nonce(nonce_store)
 #   logging.debug("versiooon noncee%s",str(noncee))
    ...
    logging.debug("before get suignature")
    signature_base_string = self.get_signature_base_string()

    # lookup signature method
    signature_method = self.get_signature_method()
    #logging.debug("signature_base_strings %s",str(signature_base_string))
    #logging.debug("self.consumer %s",str(self.consumer))
    #logging.debug("self.token %s",str(self.token))
    #logging.debug("self.signature %s",str(self.signature))
  ...
  def __check_timestamp(self):
    ...
    now = int(time.time())
    logging.debug("TIMESTAMPP %s",str(timestamp))
    logging.debug("NOW %s",str(now))
    lapsed = now - timestamp
    logging.debug("lapsed %s",str(lapsed)) 
  ...
  def get_signature_base_string(self):
    ...
    logging.debug("get signaturrrrrrrrrrrrrrrrrre %s",str(self.data))
    ...
    logging.debug("PARAMS %s",str(normalized_method))
  ...
  def from_http_request(http_request, oauth_store):
    ...
    oauth_params = None
#    b = http_request.getMethod();

 #   encoded_body = urllib.urlencode(b)
    # headers
#    logging.debug("!!!!!!!!!!!!!!!!!!!!!!!!!REQUESTTTT!!!!!!!!!!!!!!!! %s",str(http_request.))
    auth_h = " "
    headers = http_request.headers
 #   logging.debug("!!!!!!!!!!!!!!!!!!!!!!!!!headerssssssssss!!!!!!!!!!!!!!!!! %s",str(headers))
    auth_header = None
    logging.debug("MARIAAAAA %s",str(headers))
    logging.debug("GRRRRRRR-RRRRRRRRR")
    ...
    if headers.has_key('HTTP_AUTHORIZATION'):
      ...
      if "Bearer" in auth_header:
          auth_h = auth_header.split(" ")[1]	
          logging.debug("HMHMN %s",str(auth_h))

    if headers.has_key('HTTP_IMC_TOKEN') or auth_h !=" ":
      
      if  auth_h !=" ":
	    auth_header = auth_h         
      else:
            auth_header = headers['HTTP_IMC_TOKEN']
      logging.debug("IMC TOKEN%s",str(auth_header))
      imc_token = str(auth_header)
      results = ''
      ...
      try:
               cursor.execute("SELECT user_id  FROM indivo_sessiontoken where token="+"'"+imc_token+"'")
      ...
      try:
               cursor.execute("SELECT expires_at  FROM indivo_sessiontoken where token="+"'"+imc_token+"'")
               result = cursor.fetchall()
               datetimestring = result[0][0]
               logging.debug("DATETIMESTRING%s",str(datetimestring))
...
#        thedate=datetime.datetime.strptime(str(datetimestring),'%y-%m-%d %H.%M.%S.%f')
        timest2=calendar.timegm(datetimestring.utctimetuple())
        timest=str(timest2)
#        dt = dateparser.parse(datetimestring)
#        timestamp = int(time.mktime(dt.timetuple()))
        #timest= str(generate_timestamp())
        logging.debug("TIMESTTT %s",str(timest))
#      key = utils.escape('chrome' or '')
 #     key += '&'
 #     key += utils.escape('chrome' or '')
 #     key_utf8 = key.encode('utf-8')
 #     text_utf8 = text.encode('utf-8')
 #     signature = hmac.new(key_utf8, text_utf8, hashlib.sha1)
      #  logging.debug("UI APP HEADERS",str(headers))
        if headers.has_key('HTTP_ADMIN'):
             headers={'AUTHORIZATION': 'OAuth oauth_nonce="'+nonce+'", oauth_timestamp="'+timest+'", oauth_version="1.0", oauth_signature_method="HMAC-SHA1", oauth_consumer_key="chrome", oauth_signature="xNEaZ5GPygTIubetVN3rA8ftZZM%3D"'}
        else:
             headers={'AUTHORIZATION': 'OAuth oauth_nonce="'+nonce+'", oauth_timestamp="'+timest+'", oauth_consumer_key="chrome", oauth_signature_method="HMAC-SHA1", oauth_version="1.0", oauth_token="'+imc_token+'",oauth_signature="Ovfj/zNOgj3sRRBGrhshw3nF0kM="'}
#      headers = {'AUTHORIZATION': 'OAuth oauth_nonce="160194429317768154391447927435",oauth_timestamp="'+timest+'",oauth_consumer_key="chrome",oauth_signature_method="HMAC-SHA1",oauth_version="1.0",oauth_token="IB9KHUX22zjD8HVR0JGK",oauth_signature="Ovfj/zNOgj3sRRBGrhshw3nF0kM="'}
        auth_header = headers['AUTHORIZATION']
    ...
    # SZ: dict . index raises error, should use find or handle by having a try blocok

    if auth_header and auth_header.find('OAuth') > -1:
      try:
        # get the parameters from the header
        logging.debug("header!!!!!!!!!!!!!!!!! %s",str(auth_header))
        oauth_params = parse_header(auth_header)
        logging.debug("OAUTH PARAMS!!!!!!!!!!!! %s",str(oauth_params))# mariaa
      except:
    ...
    logging.debug("consumeeeeeeee before %s",str(oauth_consumer_key))
    logging.debug("consumeeeeeeee before %s",str(consumer))
    ...
    if oauth_params.has_key('oauth_token') and oauth_params['oauth_token'] != "":
      logging.debug("TOKEEEEEEEEEEn %s",str(oauth_params['oauth_token']))
      ...
      logging.debug("TOKEEEEEEEEEEn %s",str(token))
      ...
      if token:
        ...
        if token:
          ...
          logging.debug("TYPEEE %s",str(type))
          #logging.debug("value of ACCESSS is %s",str(type))
        else:
