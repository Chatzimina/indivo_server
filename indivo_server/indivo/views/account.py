...
def account_set_state(request, account):
 ...
 #       account.set_state(request.POST['state'])
      if request.POST.get('state') == 'retired':

	    argList = []
            argList.append(account.email)
	    import subprocess
#	    try:
            stri=str("python /media/data/hatzimin/web/indivo_ui_server/ui/deleteAccountComplete.py > /media/data/hatzimin/web/indivo_ui_server/ui/resulttt "+account.email)
            subprocess.check_output(stri,stderr=subprocess.STDOUT, shell=True)
...
def account_search(request):
    ...
    * *contact_email*: The contact email for the account.
    ...
    account_id = ''
    if not (fullname or contact_email or username):
...
#          conn.autocommit = True
       ...
       try:
          ...
          cursor.execute("SELECT account_id FROM indivo_accountauthsystem where username="+"'"+username+"'")
          conn.commit()
	  ...
	  if result:
       ...
       if account_id != '': 
        try:
           ...
           cursor.execute("SELECT contact_email FROM indivo_account where account_id="+"'"+account_id+"'")
           conn.commit()
           result = cursor.fetchall()
           contact_email = result[0][0]
    ...
    if username:
	query_filter |= Q(contact_email__icontains = '1392bd97247546527338')
...
#@          conn.autocommit = True
          ...
          logging.debug("errorrrr %s",str(e)) 

        logging.debug("aaaaaaaaaaaaaaaaaaaa%s",account)
        account_i = str(account)#.split(" ")[0]
        account_id = account_i.split(" ")[1]
        logging.debug("adasdsadasdasdas %s",str(account_id))
        try:
           ...
           email = result[0][0]
           logging.debug("result: %s",str(email))
        except psycopg2.DatabaseError, e:
	    logging.debug("database %s",str(e))
	


        logging.debug("arglist %s",str(argList))
...
#    if account.state != UNINITIALIZED_STATE: mariaa
#        account.reset()
#    else:
    if account.state == UNINITIALIZED_STATE:
...
def account_create(request):
    ...
    * *secondary_secret_p*: ``0`` or ``1``. Whether or not to associate
    ...
    if create_p:
        ...
        new_account.role = request.POST.get('role', '')
        new_account.sharingPreferences = request.POST.get('sharingPreferences', '')
        new_account.placeOfResidence = request.POST.get('placeOfResidence', '')
        new_account.cancerDisease = request.POST.get('cancerDisease', '')
        new_account.speciality = request.POST.get('speciality', '')        
        new_account.health_organisation = request.POST.get('health_organisation', '')
        new_account.address_organisation = request.POST.get('address_organisation', '')
        new_account.pilot = request.POST.get('pilot', '')
